...
RL.PixelUI = {}
local PixelUI = RL.PixelUI

PixelUI.inited = false;
PixelUI.pixelFrame = nil;
PixelUI.pixelTexture = nil;
PixelUI.pixelText = nil;
...
local function InitMacroButton()
    if (RL.Rotation.MainRotation == nil) or (RL.Rotation.MainRotation.Macros == nil) then
        return
    end
    local macros = RL.Rotation.MainRotation.Macros
    for i = 1, #macros do
...
function PixelUI:Init()
    if PixelUI.inited then
        ...
        -- Utils.Print("PixelUI Init Start")
        InitFrame()
        InitMacroButton()
        PixelUI.inited = true
        Utils.Print("PixelUI 初始化成功")
    end
end
...
function PixelUI:HandleAction(action1, action2)
    if GetTime() < self.delay_time then
        return
    end
